class Etcd::Lease
  ...
  def grant(ttl : Int64 = @ttl, id = 0)
    response = api.post("/lease/grant", {TTL: ttl, ID: 0})
  ...
  def keep_alive(id : Int64)
    response = api.post("/lease/keepalive", {ID: id})
    body = JSON.parse(response.body)
  ...
  def timetolive(id : Int64, query_keys = false)
    response = api.post("/kv/lease/timetolive", {ID: id, keys: query_keys})
    body = JSON.parse(response.body)
  ...
  # id  Id of lease  Int64
  def revoke(id : Int64)
    response = api.post("/kv/lease/revoke", {ID: id})
  ...
  def leases
    response_body = api.post("/kv/lease/leases").body
